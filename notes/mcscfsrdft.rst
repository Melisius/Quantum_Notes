...
The method of MCSCF-srDFT is based on the separation of two electron integrals into a long-range part and a short-range part. Now the regular two electron operator is given as:
...
Here lr is long-range, sr is short-range and :math:`\mu` is a paramter that controls where the long-range and short-range interactions are separated. From Hohnenberg-Kohn it is known that the universal functional is defined as:
