...
.. math::
   \left(\hat{T}+\hat{V}_{Ne}+\hat{W}_{ee}^{\mathrm{lr},\mu}+\hat{V}_{\mathrm{Hxc}}^{\mathrm{sr},\mu}\left[n_{\Psi^{\mu}}\right]\right)\left|\Psi^{\mu}\right\rangle =\epsilon^{\mu}\left|\Psi^{\mu}\right\rangle 
   
Here :math:`\hat{V}_{\mathrm{Hxc}}^{\mathrm{sr},\mu}\left[n_{\Psi^{\mu}}\right]` is the potential energy functional, defined as:

.. math::
   \hat{V}_{\mathrm{Hxc}}^{\mathrm{sr},\mu}\left[n_{\Psi^{\mu}}\right]=\int v_{\mathrm{Hxc}}\left[n_{\Psi^{\mu}}\right]\left(r\right)dr
   
with,

.. math::
   v_{\mathrm{Hxc}}\left[n_{\Psi^{\mu}}\right]\left(r\right)=\frac{\delta\left(E_{H}^{\mathrm{sr},\mu}+E_{xc}^{\mathrm{sr},\mu}\right)}{\delta n\left(r\right)}\left[n_{\Psi^{\mu}}\right]
...
.. math::
   \lim_{\mu\rightarrow0}E_{0}=\left\langle \Psi^{\mu}\left|\hat{T}+\hat{V}_{Ne}+0\right|\Psi^{\mu}\right\rangle +E_{H}^{\mathrm{sr},0}\left[n_{\Psi^{0}}\right]+E_{xc}^{\mathrm{sr},\mu}\left[n_{\Psi^{0}}\right]
   
Which is Kohn-Sham DFT. And:

.. math::
   \lim_{\mu\rightarrow\infty}E_{0}=\left\langle \Psi^{\mu}\left|\hat{T}+\hat{V}_{Ne}+r_{12}^{-1}\right|\Psi^{\mu}\right\rangle +0+0
